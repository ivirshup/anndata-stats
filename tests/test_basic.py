...
import anndata_stats as adstats
...
def test_groupby():
    ...
    gb = adstats.GroupBy(adata_sparse, key="key")
    stats_sparse = gb.count_mean_var()
    stats_dense = adstats.GroupBy(adata_dense, key="key").count_mean_var()
    stats_pd = adstats.GroupBy(adata_dense, key="key").pd_count_mean_var()
    ...
    gb_weight = adstats.GroupBy(adata_sparse, key="key", weight="weight")
    ...
    key_set = ["v", "w"]
    mean_key_set = adstats.GroupBy(adata_sparse, key="key", key_set=key_set).mean()
    assert np.allclose(stats_sparse.mean.loc[key_set], mean_key_set)

    gb_explode = adstats.GroupBy(adata_sparse, key="tuple_key", explode=True)
